...
class CodeBook{
  private:
    ...
    static const int tbin = nbin*nbin;
  ...
  public:
    CodeBook(){
        ...
        for(int i = 0; i < tbin; i++){
            ...
            colorBin[i] = QuantizedBin(Vec3b(150, g*sbin+sbin/2, r*sbin+sbin/2));
            colorBinIdx[i] = i;
        }
        isSorted = true;
    }
...
int main(int argc, char *argv[]){
    ...
    vid.release();
    Mat bg = background.get_background_model();
    cvtColor(bg, bg, CV_Lab2BGR);
    imshow("Learned Background", bg);
    ...
    elapsetime = (int)((cv::getTickCount()-t0)/cv::getTickFrequency());
    printf("%d min %d sec \n\n", elapsetime/60, elapsetime%60);
