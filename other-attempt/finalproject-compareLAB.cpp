...
class CodeBook{
  ...
  public:
    ...
    bool check_color(QuantizedBin &bin, Vec3b &rgb, Vec3b &lab){
        bool match = true;
        for(int i = 0; i < 3; i++){
            if(abs(bin.color[i]-rgb[i]) > 40)
                match = false;
        }
        if(match)
            return true;

        if(abs(bin.colorLAB[0]-lab[0]) < 80 &&
            abs(bin.colorLAB[1]-lab[1]) < 6 &&
            abs(bin.colorLAB[2]-lab[2]) < 6)
            return true;

        return false;
    }

    bool is_background_color(Vec3b &rgb, Vec3b &lab){
        if(!isSorted)
            binSort();

        QuantizedBin &dominant = colorBin[0];

        if(check_color(dominant, rgb, lab))
            return true;
        ...
        for(int i = 1; i < 10; i++){
            ...
            if(colorBin[i].lambda > 300)
                continue;
            if(check_color(colorBin[i], rgb, lab))
                return true;
        }
        return false;
    }
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                ...
                foreground.at<uchar>(i, j) = 255 *
                    !pixelCodeBook[i*cols+j].is_background_color(frame.at<Vec3b>(i, j), lab.at<Vec3b>(i, j));
            }
        }
        return foreground;
    }
    ...
    void computeVelocity(vector<ForegroundObject> &curObjects){
        ...
        for(int i = 0; i < curObjects.size(); i++){
            for(int j = 0; j < objects.size(); j++){
                if( abs(curObjects[i].center.x - objects[j].center.x - objects[j].velocity_x) < 7 &&
                    abs(curObjects[i].center.y - objects[j].center.y - objects[j].velocity_y) < 7){
                    //matched
